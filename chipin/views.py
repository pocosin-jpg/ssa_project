...
def edit_comment(request, comment_id):
    ...
    return render(
        request, "chipin/edit_comment.html", {"form": form, "comment": comment}
    )
...
def delete_comment(request, comment_id):
    comment = get_object_or_404(Comment, id=comment_id)
    if (
        comment.user == request.user or request.user == comment.group.admin
...
def home(request):
    ...
    user_join_requests = GroupJoinRequest.objects.filter(
        user=user
    )  # Get join requests sent by the user
    available_groups = Group.objects.exclude(members=user).exclude(
        join_requests__user=user
    )  # Get groups the user is not a member of and the user has not requested to join
    context = {
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if Group.objects.filter(name=group_name).exists():
                messages.error(
                    request,
                    f'A group with the name "{group_name}" already exists. Please choose a different name.',
                )
            else:
                group = form.save()
                logger.info(
...
def group_detail(request, group_id, edit_comment_id=None):
    group = get_object_or_404(Group, id=group_id)
    comments = group.comments.all().order_by(
        "-created_at"
...
def invite_users(request, group_id):
    group = get_object_or_404(Group, id=group_id)
    users_not_in_group = CustomUser.objects.exclude(
    ...
    if request.method == "POST":
        ...
        if invited_user in group.invited_users.all():
            messages.info(
                request, f"{invited_user.profile.nickname} has already been invited."
            )
        else:
            group.invited_users.add(invited_user)
            messages.success(
                request, f"Invitation sent to {invited_user.profile.nickname}."
            )
        return redirect("chipin:group_detail", group_id=group.id)
    return render(
        request,
...
def accept_invite(request, group_id):
    ...
    if user_id:
        ...
        if invited_user in group.members.all():
            messages.info(
                request,
                f'{invited_user.profile.nickname} is already a member of the group "{group.name}".',
            )
...
def vote_on_join_request(request, group_id, request_id, vote):
    ...
    if total_votes / total_members >= 0.6:
        ...
        messages.success(
            request,
            f"{join_request.user.profile.nickname} has been approved to join the group!",
        )
